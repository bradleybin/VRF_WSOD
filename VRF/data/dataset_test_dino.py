...
import numpy as np
try:
    from . import transform_test_dino
except:
    import transform_test_dino
...
class Data(Dataset):
    def __init__(self, cfg):
        with open(cfg.datapath+'/'+cfg.mode+'.txt', 'r') as lines:
            ...
            for line in lines:
                ...
                tokenth = '/home/xubinwei/code/wsod2/image_processing/TOKEN/TokenCut/duts_test_train_token_similarity_40_40_384/' + line.strip().replace(
                    '.jpg', '.npy')
                self.samples.append([imagepath, maskpath, tokenth])

        if cfg.mode == 'train':
            self.transform = transform_test_dino.Compose(transform_test_dino.Normalize(mean=cfg.mean, std=cfg.std),
                                                    transform_test_dino.Resize(320, 320),
                                                    transform_test_dino.RandomHorizontalFlip(),
                                                    transform_test_dino.RandomCrop(320, 320),
                                                    transform_test_dino.ToTensor())
        elif cfg.mode == 'test':
            self.transform = transform_test_dino.Compose(transform_test_dino.Normalize(mean=cfg.mean, std=cfg.std),
                                                    transform_test_dino.Resize(320, 320),
                                                    transform_test_dino.ToTensor())
    ...
    def __getitem__(self, idx):
        imagepath, maskpath, tokenth = self.samples[idx]
        #print('imagepath!!!!!!!!!!!', imagepath)
        ...
        image, mask, token         = self.transform(image, mask, token)
        ...
        # mask[mask == 0.] = 255.
        # mask[mask == 2.] = 0.
        return image, mask, (H, W), maskpath.split('/')[-1], trans_token
